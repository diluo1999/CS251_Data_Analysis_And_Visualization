...
class Data:
    ...
    def read(self, filepath):
        ...
        with open(filepath, 'r') as csvfile:
            ...
            headersTemp = reader.__next__()
            typesTemp = reader.__next__()
            ...
            if len(self.types) == 0:
                print('No numeric data in the file or the second row doesn\'t have the correct information: data type')
                return None

            for j in indexList:
                self.headers.append(headersTemp[j].strip())

            for rowTemp in reader:
                row = []
                for k in indexList:
                    row.append(float(rowTemp[k]))
                # row=[float(rowTemp[k]) for k in indexList]
    ...
    def __str__(self):
        ...
        s += 'Headers:\n' + '\t'.join(map(str, self.get_headers()))
        s += '\nTypes:\n' + '\t'.join(map(str, self.get_types()))
        s += '\n-------------------------------\n'
        if self.get_num_samples() > 5:
            s += 'Showing first 5/' + str(self.get_num_samples()) + ' rows.\n'
            for i in range(5):
                s += '\t'.join(map(str, self.data[i, :].tolist())) + '\n'    
        else:
    ...
    def select_data(self, headers, rows=[]):
        ...
        if type(headers) == list:
            headerInd = self.get_header_indices(headers)
            if len(headerInd) == 0:
                print('select_data(): The variable names in `headers` have no correponding header in data.')
            if len(rows) == 0:
                return self.data[np.ix_(np.arange(self.get_num_samples()), headerInd)]
            else:
                return self.data[np.ix_(rows, headerInd)]
        elif type(headers) == str:
            headerInd = self.get_mappings()[headers]
            if len(rows) == 0:
                return self.data[:, headerInd][:, np.newaxis]
            else:
                return self.data[np.ix_(rows, [headerInd])]
