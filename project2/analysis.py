...
class Analysis:
    ...
    def pair_plot(self, data_vars, fig_sz=(12, 12), title=''):
        ...
        # Added in Project 2: make each plot has the same range of x and y
        # gloMax = np.max(selected_variables)
        # gloMin = np.min(selected_variables)
        ...
        for i in range(M):
            for j in range(M):
                ...
                # Added in Proj 2
                # axes[i,j].set_xlim((gloMin-1, gloMax+1))
                # axes[i,j].set_ylim((gloMin-1, gloMax+1))
        return fig, axes
