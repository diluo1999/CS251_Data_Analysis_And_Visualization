...
class PCA_COV:
    ...
    def __init__(self, data):
        ...
        #   Means of each orignal data variable (after normalization)
        self.orig_means = None

        # orig_mins: ndarray. shape=(num_selected_vars,)
        #   Mins of each orignal data variable (before normalization)
        self.orig_mins = None

        # orig_scales: ndarray. shape=(num_selected_vars,)
        #   Ranges of each orignal data variable (before normalization)
    ...
    def elbow_plot(self, num_pcs_to_keep=None):
        ...
        plt.xlabel('Num of Top PCs included')
    ...
    def loading_plot(self):
        ...
        for i in range(len(pc1)):
            ...
            y = pc2[i]
            plt.plot([0, x], [0, y])
            plt.annotate(self.vars[i], xy=(x, y))

        plt.xlabel("PC1")
        plt.ylabel("PC2")
        plt.title('Loading Plot of Top 2 PCs')
    ...
    def pca_then_project_back(self, top_k):
        ...
        '''
        A_proj = self.pca_project(np.arange(top_k))

        vecs_top = self.e_vecs[:, :top_k]

        orig_data = A_proj @ vecs_top.T + self.orig_means

        if self.normalized:
            orig_data = orig_data * self.orig_scales + self.orig_mins

        return orig_data

    # Extension
    def loading_plot_generic(self, pc_a, pc_b):
        '''Create a loading plot of 2 in all PC eigenvectors

        Parameters:
        -----------
        pc_a: int. The index of first PC to be investigated
        pc_b: int. The index of second PC to be investigated
        (Index of PCs: 1, 2, 3 ...)

        TODO:
        - Plot a line joining the origin (0, 0) and corresponding components of 2 in all PC eigenvectors.
            Example: If e_1 = [0.1, 0.3] and e_2 = [1.0, 2.0], you would create two lines to join
            (0, 0) and (0.1, 1.0); (0, 0) and (0.3, 2.0).
            Number of lines = num_vars

        '''
        pc1 = self.e_vecs[:, pc_a-1]
        pc2 = self.e_vecs[:, pc_b-1]
        ...
        plt.xlabel("PC"+str(pc_a))
        plt.ylabel("PC"+str(pc_b))
        plt.title('Loading Plot of PC' + str(pc_a) + ' and PC' + str(pc_b))