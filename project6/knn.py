...
class KNN:
    ...
    def predict(self, data, k):
        ...
        for i in range(data.shape[0]):
            ...
            value, count = np.unique(self.classes[x], return_counts=True)
            ind = np.argmax(count)
            pred.append(value[ind])
    ...
    def accuracy(self, y, y_pred):
        ...
        '''
        n = y.shape[0]
        prop = (n - np.count_nonzero(y - y_pred)) / n
        return prop
    ...
    def plot_predictions(self, k, n_sample_pts):
        ...
        '''
        samp_vec = np.linspace(-40,40, num = n_sample_pts)
        x, y = np.meshgrid(samp_vec, samp_vec)

        pairs = np.column_stack((x.flatten(),y.flatten()))
        pairs = pairs.reshape(n_sample_pts*n_sample_pts, self.exemplars.shape[1])

        pred = self.predict(pairs, k)
        pred=np.array(pred)
        pred = pred.reshape(n_sample_pts,n_sample_pts)
        cmap = ListedColormap(cartocolors.qualitative.Safe_5.mpl_colors)
        color = plt.pcolormesh(x,y,pred,cmap = cmap)
        plt.colorbar(color)
        plt.xlabel('x')
        plt.ylabel('y')
